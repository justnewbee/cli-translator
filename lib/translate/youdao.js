...
module.exports = text => new Promise(function(resolve, reject) {
	let prefYD = pref("youdao") || {};
	
	request({
		url: "http://fanyi.youdao.com/openapi.do",
		qs: {
			key: prefYD.key || "445042457",
			keyfrom: prefYD.keyFrom || "cli-translator",
			type: "data",
			doctype: "json",
			version: "1.1",
			q: text
		...
		try {
			...
			if (result.errorCode === 0) {
				let {translation, basic, web} = result;
				basic = basic || {};
				
				resolve({
					phonic: _helper.parsePhonic(basic.phonetic, basic["uk-phonetic"], basic["us-phonetic"]),
					translations: basic.explains && basic.explains.length ? basic.explains.map(v => /^([\w \.&]+\.) (.+)$/.test(v) ? {
						pos: RegExp.$1,
